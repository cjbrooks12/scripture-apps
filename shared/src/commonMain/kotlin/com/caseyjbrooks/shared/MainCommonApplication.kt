...
public fun MainCommonApplication(
    ...
    WithKoinApplication(koin) {
        WithRouter {
            val initialFeatureModule: FeatureModule = LocalKoin.current.get()
            WithSizeClass(
                desktopContent = { DesktopLayout(initialFeatureModule) },
                phoneContent = { PhoneLayout(initialFeatureModule) },
            )
        }
    }
}
