package com.copperleaf.biblebits.minigames.verses.typeverse
...
@Composable
fun TypeVerseWithLetterDashesMinigame(
    ...
    Column {
        ...
        val verseHint by derivedStateOf {
            reformatText(
                text = verseText,
                reformatWord = { it.first() + CharArray(it.length - 1) { '_' }.concatToString() },
                random = Random(1),
                threshold = 0.5f,
                progress = progress
            )
        ...
        Text("Type out the verse using the lettered-dashes as a guide", style = MaterialTheme.typography.headlineMedium)
